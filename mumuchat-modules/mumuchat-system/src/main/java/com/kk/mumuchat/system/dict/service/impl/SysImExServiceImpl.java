...
import com.kk.mumuchat.system.dict.domain.correlate.SysImExCorrelate;
...
public class SysImExServiceImpl extends BaseServiceImpl<SysImExQuery, SysImExDto, SysImExCorrelate, ISysImExManager> implements ISysImExService {
    ...
    public CacheModel getCacheModel() {
        return new CacheModel(ConfigConstants.CacheType.SYS_IM_EX_KEY);
    }
    ...
    public Boolean syncCache() {
        ...
        Map<String, SysImExDto> enterpriseImExMap = MapUtil.buildListToMap(enterpriseTypeList, SysImExDto::getCode, Function.identity());
    ...
    public void clearCache() {
        Collection<String> keys = redisService.keys(ConfigConstants.CacheKey.IM_EX_KEY.getCacheName(StrUtil.ASTERISK));
     ...
     */
    @Override
    public void refreshCache(OperateConstants.ServiceType operate, RedisConstants.OperateType cacheOperateType, SysImExDto dto, Collection<SysImExDto> dtoList,
        ...
        // 默认缓存管理方法
        super.refreshCache(operate, cacheOperateType, dto, dtoList, cacheKey, isTenant, SysImExDto::getCode, cacheValueFun);
        // 路由缓存管理方法
        if (SecurityUtils.isCommonTenant()) {
            ConfigConstants.CacheType routeCacheKey = ConfigConstants.CacheType.ROUTE_IM_EX_KEY;
            super.refreshCache(operate, cacheOperateType, dto, dtoList, routeCacheKey.getCode(), routeCacheKey.getIsTenant(), SysImExDto::getCode, Function.identity());
        }
    }
}