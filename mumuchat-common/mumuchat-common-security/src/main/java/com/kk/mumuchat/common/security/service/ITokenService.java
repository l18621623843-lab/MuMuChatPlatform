...
import com.kk.mumuchat.common.cache.utils.EnterpriseUtil;
...
import static com.kk.mumuchat.common.cache.constant.CacheConstants.ACCESS_TIME;
import static com.kk.mumuchat.common.cache.constant.CacheConstants.EXPIRATION;
import static com.kk.mumuchat.common.cache.constant.CacheConstants.REFRESH_TIME;
...
public interface ITokenService<User, LoginUser extends BaseLoginUser<User>> extends Ordered {
     ...
     * @return JWT加密串
     */
    default String buildJwtToken(LoginUser loginUser) {
    ...
    default Map<String, Object> createToken(LoginUser loginUser) {
        ...
        rspMap.put(SecurityConstants.BaseSecurity.ACCESS_TOKEN.getCode(), buildJwtToken(loginUser));
        rspMap.put(SecurityConstants.BaseSecurity.EXPIRES_IN.getCode(), getRefreshExpireTime());
        return rspMap;
    }
    ...
    default void createTokenCache(OAuth2Authorization authorization, LoginUser loginUser, Map<String, Object> loginMap) {
        RedisJavaService redisJavaService = SpringUtil.getBean(RedisJavaService.class);
        if (ObjectUtil.isNotNull(loginUser.getAccessToken())) {
            redisJavaService.setCacheObject(loginUser.getAccessToken(), authorization, getAccessExpireTime(), TimeUnit.MINUTES);
        }
        ...
        if (ObjectUtil.isNotNull(loginUser.getStateToken())) {
            redisJavaService.setCacheObject(loginUser.getStateToken(), authorization, getAccessExpireTime(), TimeUnit.MINUTES);
        }
        if (ObjectUtil.isNotNull(loginUser.getCodeToken())) {
            redisJavaService.setCacheObject(loginUser.getCodeToken(), authorization, getAccessExpireTime(), TimeUnit.MINUTES);
        }
    }
