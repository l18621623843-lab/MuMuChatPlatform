package com.kk.mumuchat.common.security.handler;

import com.kk.mumuchat.common.core.constant.basic.HttpConstants;
import com.kk.mumuchat.common.core.exception.DemoModeException;
...
public class GlobalExceptionHandler {
    ...
    public AjaxResult handleNoResourceFoundException(Exception e, HttpServletRequest request) {
        String requestURI = request.getRequestURI();
        log.warn("请求地址'{}', 资源不存在, 异常原因: {}", requestURI, e.getMessage());
        return AjaxResult.error(e.getMessage());
    }
    ...
    public Object handleMethodArgumentNotValidException(MethodArgumentNotValidException e) {
        log.error(e.getMessage(), e);
        String message = Optional.of(e).map(BindException::getBindingResult).map(Errors::getFieldError)
                .map(DefaultMessageSourceResolvable::getDefaultMessage).orElse(null);
        return AjaxResult.error(message);
    }
