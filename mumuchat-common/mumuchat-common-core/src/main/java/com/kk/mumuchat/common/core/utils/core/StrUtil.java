...
import com.kk.mumuchat.common.core.utils.core.pool.StrPool;
...
public class StrUtil extends cn.hutool.core.util.StrUtil implements StrPool {
     ...
     *
     * @param expected 期望值
     * @param actual   可变参数，要比较的字符串数组
     * @return 所有值都相等返回 true，否则返回 false
     */
    public static boolean equalsAll(CharSequence expected, CharSequence... actual) {
        if (actual == null || actual.length == 0) {
            return false; // 空参数或null视为不匹配
        }
        for (CharSequence str : actual) {
            if (notEquals(str, expected)) {
                return false;
            }
        }
        return true;
     ...
     * @param consumer 操作方法
     */
    public static void setStrIfBlank(String str, Consumer<String> consumer) {
        if (isBlank(str)) {
            consumer.accept(str);
        }
    }
     ...
     * @param runnable 操作方法
     */
    public static void setStrIfBlank(String str, Runnable runnable) {
        if (isBlank(str)) {
            runnable.run();
        }
    }
     ...
     * @param consumer 操作方法
     */
    public static void setStrIfNotBlank(String str, Consumer<String> consumer) {
        if (isNotBlank(str)) {
            consumer.accept(str);
        }
    }
     ...
     * @param runnable 操作方法
     */
    public static void setStrIfNotBlank(String str, Runnable runnable) {
        if (isNotBlank(str)) {
            runnable.run();
        }
    }
     ...
     * @param elseStr 代替值
     */
    public static String getStrOrBlankElse(String str, String elseStr) {
        return isBlank(str) ? elseStr : str;
    }

    /**
     * 当值不为空时，使用指定函数处理该值；当值为空时，返回指定的替代值
     *
     * @param str      待处理的字符串
     * @param func     处理函数，当str不为空时应用此函数
     * @param emptyStr 当str为空时返回的替代值
     * @return 处理后的字符串或替代值
     */
    public static String getStrByFuncOrBlankElse(String str, Function<String, String> func, String emptyStr) {
        if (isBlank(str)) {
            return emptyStr;
        }
