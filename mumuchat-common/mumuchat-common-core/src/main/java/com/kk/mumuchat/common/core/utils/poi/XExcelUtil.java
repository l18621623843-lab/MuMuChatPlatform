package com.kk.mumuchat.common.core.utils.poi;
...
import com.kk.mumuchat.common.core.utils.core.ReflectUtil;
import com.kk.mumuchat.common.core.utils.core.StrUtil;
import com.kk.mumuchat.common.core.utils.file.FileTypeUtil;
import com.kk.mumuchat.common.core.utils.file.ImageUtil;
...
public class XExcelUtil {
    ...
    private void errorMsg(HttpServletResponse response, Exception e) {
        try {
            response.reset();
            response.setContentType("application/json;charset=utf-8");
            AjaxResult result = AjaxResult.error("导出Excel失败:" + e.getMessage());
            response.getWriter().write(JSON.toJSONString(result));
        } catch (IOException ioException) {
            log.error("导出Excel异常时返回错误信息失败", ioException);
        }
    }
    ...
    public static class ExcelInfo {
        ...
        private void createWorkbook() {
            // 根据数据量动态调整窗口大小，最小100，最大1000
            int windowSize = Math.max(100, Math.min(1000, this.sheetInfoList.stream()
                    .mapToInt(sheet -> sheet.getList().size())
                    .max()
                    .orElse(500)));
        ...
        public static class SheetInfo<C> {
            ...
            private Cell addCell(Workbook wb, FieldInfo fieldInfo, Row row, C vo, int column) {
                ...
                try {
                    ...
                    log.error("导出Excel失败，行号：{}，列号：{}，字段：{}", row.getRowNum(), column, fieldInfo.getName(), e);
                    throw new UtilException("导出Excel在字段" + fieldInfo.getName() + "处失败：" + e.getMessage());
                }
                return cell;
            }
            ...
            private void setCellVo(Object value, FieldInfo fieldInfo, Cell cell) {
                if (ColumnType.STRING == fieldInfo.getCellType()) {
                    ...
                    // 对于任何以表达式触发字符 =-+@开头的单元格，直接添加制表符前缀，防止CSV注入。
            ...
            private void addStatisticsRow() {
                if (MapUtil.isNotEmpty(this.statistics)) {
                    ...
                    for (Integer key : keys) {
                        ...
                        cell.setCellStyle(this.styles.get("total"));
                        cell.setCellValue(DOUBLE_FORMAT.get().format(this.statistics.get(key)));
                    }
            ...
            private void getValueByFieldInfo(FieldInfo fieldInfo, Object entity, Object val, int index) {
                ...
                // 空值不考虑匹配
                if (StrUtil.isBlank(ConvertUtil.toStr(val))) {
                    return;
                }
            ...
            public static class FieldInfo {
                ...
                public static class ModelInfo {
                    ...
                    private static String convertByExp(String propertyValue, List<ModelInfo> converterExp, String separator) {
                        ...
                        Map<String, String> converterMap = MapUtil.buildListToMap(converterExp, ModelInfo::getValue, ModelInfo::getLabel);
                    ...
                    private static String reverseByExp(String propertyValue, List<ModelInfo> converterExp, String separator) {
                        Map<String, String> converterMap = MapUtil.buildListToMap(converterExp, ModelInfo::getLabel, ModelInfo::getValue);
                        List<String> strList = StrUtil.split(propertyValue, separator);
                        StringBuilder propertyString = new StringBuilder();
                        for (String item : strList) {
                            propertyString.append(converterMap.getOrDefault(StrUtil.cleanBlank(item), StrUtil.EMPTY));
