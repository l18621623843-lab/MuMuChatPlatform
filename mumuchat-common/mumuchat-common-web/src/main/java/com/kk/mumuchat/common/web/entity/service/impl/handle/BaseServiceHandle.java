package com.kk.mumuchat.common.web.entity.service.impl.handle;
...
import com.kk.mumuchat.common.redis.service.RedisJavaService;
...
public class BaseServiceHandle<Q extends BaseEntity, D extends BaseEntity, C extends Enum<? extends Enum<?>> & CorrelateService, IDG extends IBaseManager<Q, D>> extends BaseCorrelateHandle<D, C> implements BaseCacheHandle<D> {
    ...
    @Resource
    private RedisJavaService redisJavaService;
     ...
     */
    @Override
    public void refreshCache(OperateConstants.ServiceType operate, RedisConstants.OperateType cacheOperateType, D dto, Collection<D> dtoList,
        ...
        Collection<D> dataList = operate.isBatch() ? dtoList : CollUtil.singleList(dto);
        cacheService.syncCacheData(cacheOperateType, cacheKeyName, dataList, cacheKeyFun, cacheValueFun);
    }
    ...
    protected C getBasicCorrelate(CorrelateConstants.ServiceType serviceType) {
        ...
        if (!redisJavaService.hasKey(cacheKey)) {
            ...
            redisJavaService.setCacheMap(cacheKey, cacheMap);
        }
        C correlate = redisJavaService.getCacheMapValue(cacheKey, serviceType.getCode());
        return switch (serviceType) {
            case SELECT, ADD, EDIT, DELETE, CACHE_REFRESH, EDIT_STATUS -> correlate;
            case SELECT_LIST, SELECT_ID_LIST, SELECT_ID_SINGLE ->
                    ObjectUtil.isNotNull(correlate) ? correlate : redisJavaService.getCacheMapValue(cacheKey, CorrelateConstants.ServiceType.SELECT.getCode());
            case BATCH_ADD ->
                    ObjectUtil.isNotNull(correlate) ? correlate : redisJavaService.getCacheMapValue(cacheKey, CorrelateConstants.ServiceType.ADD.getCode());
            case BATCH_EDIT ->
                    ObjectUtil.isNotNull(correlate) ? correlate : redisJavaService.getCacheMapValue(cacheKey, CorrelateConstants.ServiceType.EDIT.getCode());
            case BATCH_DELETE ->
                    ObjectUtil.isNotNull(correlate) ? correlate : redisJavaService.getCacheMapValue(cacheKey, CorrelateConstants.ServiceType.DELETE.getCode());
        };
    }
}
