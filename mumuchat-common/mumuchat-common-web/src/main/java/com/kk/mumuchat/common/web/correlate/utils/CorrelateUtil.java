...
import com.kk.mumuchat.common.web.correlate.domain.Remote;
import com.kk.mumuchat.common.web.correlate.handle.CorrelateBaseHandle;
import com.kk.mumuchat.common.web.correlate.handle.CorrelateDirectHandle;
import com.kk.mumuchat.common.web.correlate.handle.CorrelateIndirectHandle;
import com.kk.mumuchat.common.web.correlate.handle.CorrelateRemoteHandle;
...
public final class CorrelateUtil {
     ...
     * @return 数据对象集合
     */
    public static <D extends BaseEntity, Coll extends Collection<D>, C extends Enum<? extends Enum<?>> & CorrelateService> Coll subCorrelates(Coll dtoList, C correlateEnum) {
        startCorrelates(correlateEnum);
        getCorrelates().stream().filter(relation -> ObjectUtil.equals(CorrelateConstants.SubOperate.SELECT, relation.getOperateType()))
    ...
    public static <Correlate extends BaseCorrelate> List<Correlate> getCorrelates() {
        List<Correlate> relations = (List<Correlate>) Optional.ofNullable(THREAD_LOCAL.get()).map(Deque::poll).orElseGet(ArrayList::new);
