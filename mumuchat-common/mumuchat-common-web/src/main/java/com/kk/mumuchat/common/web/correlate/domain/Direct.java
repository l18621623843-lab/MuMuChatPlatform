package com.kk.mumuchat.common.web.correlate.domain;
...
public final class Direct<D extends BaseEntity, S extends BaseEntity> extends BaseCorrelate<Direct.ORM> {
     ...
     * @param subInfoFun 从数据关联从对象
     */
    public Direct setSubInfoFun(SFunction<D, ?> subInfoFun) {
     ...
     * @param copyInfoFun 数据复制关联对象
     */
    public Direct setCopyInfoFun(List<CopyFun<D, S>> copyInfoFun) {
        if (ObjectUtil.notEqual(CorrelateConstants.SubOperate.ADD, this.operateType) && ObjectUtil.notEqual(CorrelateConstants.SubOperate.EDIT, this.operateType)) {
    ...
    @Data
    @EqualsAndHashCode(callSuper = true)
    public final static class ORM extends BaseCorrelate.ORM<ORM> {
