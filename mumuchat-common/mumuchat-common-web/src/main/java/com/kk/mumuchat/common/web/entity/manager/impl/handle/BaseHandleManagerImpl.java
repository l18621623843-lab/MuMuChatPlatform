...
import com.kk.mumuchat.common.core.utils.sql.SqlUtil;
...
public class BaseHandleManagerImpl<Q extends P, D extends P, P extends BaseEntity, PM extends BaseMapper<Q, D, P>, CT extends BaseConverter<Q, D, P>> {
    ...
    protected LambdaQueryWrapper<P> selectListQuery(Q query) {
        return new LambdaQueryWrapper<P>(query)
                .func(ObjectUtil.isNotNull(query), fc1 -> fc1
                        .func(fc -> SqlUtil.handleTimeRangeQuery(fc, query.getCreateTimeQuery(), P::getCreateTime))
                        .func(fc -> SqlUtil.handleTimeRangeQuery(fc, query.getUpdateTimeQuery(), P::getUpdateTime))
                );
    }
