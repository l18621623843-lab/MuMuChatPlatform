package com.kk.mumuchat.common.web.correlate.handle;
...
public final class CorrelateRemoteHandle extends CorrelateBaseHandle {
    ...
    private static <D extends BaseEntity, S extends BaseEntity, Coll extends Collection<D>> void assembleRemoteBuild(D dto, Coll dtoList, Remote<D, S> remote) {
        ...
        // 子查询进行数据关联操作
        List<S> remoteList = CollUtil.selectListToBatch(findInSet, (codeList) -> {
            R<List<S>> subListR = SpringUtil.getBean(ormRemote.getRemoteService()).selectListByIdsInner(codeList);
            resultFail(subListR);
            return subListR.getData();
        });

        if (ObjectUtil.isNotNull(dto)) {
            setSubField(dto, remoteList, ormRemote.getSubDataRow(), ormRemote.getMergeType(), ormRemote.getMainKeyField(), ormRemote.getSlaveKeyField(), ormRemote.getSubInfoField());
        } else if (CollUtil.isNotEmpty(dtoList)) {
            setSubField(dtoList, remoteList, ormRemote.getSubDataRow(), ormRemote.getMergeType(), ormRemote.getMainKeyField(), ormRemote.getSlaveKeyField(), ormRemote.getSubInfoField());
        }
    }
